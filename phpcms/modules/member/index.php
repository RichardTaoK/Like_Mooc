...
class index extends foreground {
	...
	public function register() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$userinfo['username'] = (isset($_POST['username']) && $_POST['username']) ? $_POST['username'] : exit('0');
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				$userid = $this->db->insert($userinfo, 1);  //往数据库里写数据
				// showmessage(L('operation_failure'), HTTP_REFERER);
				// showmessage('注册成功', 'index.php?m=member&c=index&a=login');
				// $this->_init_phpsso();//网站群，只要登录一块就全网站都行
